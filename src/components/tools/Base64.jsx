...
export default function Base64() {
    ...
    return <div className="flex flex-col justify-center items-center ">
        <div className="flex flex-col justify-center items-center bg-white">
            <div className="flex flex-col items-center justify-center  rounded-3xl md:w-2xl lg:w-3xl shadow-2xl shadow-blue-300 inset-shadow-sm inset-shadow-white">
                <div className="font-bold text-2xl p-2 italic font-mono ">
                    Base<span className="text-[#2a41e8]">64</span> Encoder<span className="text-[#2a41e8]">/</span>Decoder
                </div>
                <div className="border-t border-slate-300 w-full flex flex-col justify-center">
                    <div className="flex justify-center my-2">
                        <div className="bg-gray-400  p-1 rounded-2xl ">
                            ...
                            }} className={`py-1 px-8 mr-1 rounded-2xl font-medium text-white text-lg ease-in-out transition-all   ${encoder ? "bg-[#2a41e8] inset-shadow-sm inset-shadow-white  shadow-lg shadow-blue-500/50" : ""}`}>
                                Encoder
                            </button>
                            <button onClick={() => {
                                if (encoder) setEncoder(false);
                            }} className={`py-1 px-8 ml-1 rounded-2xl text-white font-medium text-lg ease-in-out transition-all ${!encoder ? "bg-[#2a41e8] inset-shadow-sm inset-shadow-white  shadow-lg shadow-blue-500/50" : ""}`}>
                                Decoder
                            </button>
                        </div>
                    </div>
                    ...
                    <div className=" flex justify-between px-5">
                        ...
                        <button onClick={() => {
                            navigator.clipboard.writeText(coded)
                                .then(() => { })
                        }} className="p-2 mb-2 border rounded-3xl w-20  border-blue-300 hover:bg-blue-200 text-blue-800 font-bold italic cursor-pointer flex justify-center items-center"><span>copy</span> <img src="https://img.icons8.com/?size=100&id=AGLXTiVVflkt&format=png&color=228BE6" className="h-5" alt="" /> </button>
                    </div>
                </div>
            </div>
        </div>
